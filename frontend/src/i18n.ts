...
const zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      finallyLabel: "收尾任务（finally）",
      finallyHint: "主链结束后执行，日志单独分组。",
      addFinallyAction: "添加收尾动作",
      ...
      actionUpdateProfile: "修改个人简介",
      labelProfileAbout: "新简介",
      profileAboutPlaceholder: "留空 = 清空简介",
      profileAboutHint: "整段替换原简介，留空即清空。上限 70 字（Premium 140）。",
      actionOpenMiniApp: "打开小程序（过 Cloudflare）",
      actionOpenMiniAppUrl: "打开小程序链接（过 Cloudflare）",
      actionOpenBotMenuApp: "打开机器人菜单小程序（左下角按钮）",
      actionOpenUrl: "打开网页（过 Cloudflare）",
      labelContent: "消息/命令",
      ...
      labelCfChallenge: "过 Cloudflare 验证（我不是机器人）",
      cfChallengeHint: "当点击后出现需要打开的网址（如“我不是机器人”按钮或链接），用无头浏览器打开并通过 Cloudflare 验证以完成签到。需要镜像内置 Chromium。",
      ...
      inAppButtonPlaceholder: "例：delay(10000) > 参与抽奖|Join giveaway > 确认",
      inAppButtonHint: "打开小程序页面后按顺序执行，用 > 或 -> 分隔。可用步骤：①按钮文字，如 立即签到（优先匹配真正的按钮/链接，而不是同名的说明文字）；多种写法用 | 分隔，任意一个匹配即点击，如 参与抽奖|加入抽奖|Join giveaway，适用于小程序会按语言切换按钮文字的情况；②css: 选择器，如 css:#checkin-btn；③delay(10000) 等待 10 秒再执行下一步（也可写 delay(10s)），适用于页面加载较慢的小程序；④scroll(0, 800) 按像素滚动页面（第一个数字为横向 x，第二个为纵向 y；只写一个数字表示纵向，如 scroll(800)；也可写 scroll(y=800)、scroll(x=-200)；负数为反向滚动），用于把屏幕外的按钮滚到可见区域，数值超过页面长度时直接滚到尽头，例如 scroll(0, 99999) 滚到底部；⑤{aiBtn} 截图后由 AI 判断该点哪个按钮，可用 {aiBtn:签到按钮} 补充说明；⑥{input} 自动计算算术验证码并填入；⑦{aiInput} 由 AI 读题并填入。常用组合：delay(10000) > {aiBtn}（等页面加载完再让 AI 找按钮）；scroll(0, 99999) > 立即签到（先滚到底部再点按钮）。留空则自动识别签到类按钮。按钮已置灰或显示已完成时不会点击；若只点到普通文字且页面没有反应，该操作会报错而不是误报成功",
    ...
    web: {
      ...
      labelTimeout: "超时（毫秒）",
      timeoutHint: "元素在此时间内未出现则该步骤失败。留空或 0 表示 30 秒",
  ...
  settings: {
    ...
    feishuSection: "飞书通知",
    feishuHint: "任务结束时同时推送到飞书自定义机器人，与上方 Telegram 推送共享「通知时机」。",
    labelFeishuWebhook: "Webhook 地址",
    feishuWebhookPlaceholder: "https://open.feishu.cn/open-apis/bot/v2/hook/...",
    feishuWebhookHint: "在飞书群中添加「自定义机器人」并复制其 Webhook 地址。留空则保留已保存的地址。",
    feishuConfigured: "已配置飞书推送",
    labelFeishuSecret: "签名密钥（可选）",
    feishuSecretPlaceholder: "机器人开启签名校验时填写",
    feishuSecretPlaceholderSet: "已保存（留空保持不变）",
    feishuSecretHint: "若机器人在安全设置中开启了「签名校验」，需在此填写密钥；未开启则留空。",
    feishuTestBtn: "发送测试",
    feishuTesting: "发送中…",
    feishuTestSent: "飞书测试通知已发送",
    feishuTestFailed: "飞书测试通知发送失败",
    cfSolver: {
      title: "浏览器设置",
      ...
      clearProfilesHint: "删除各出口浏览器保存的 Cookie 与缓存（不影响浏览器指纹，各出口仍是同一台“机器”）；浏览器被强杀后残留的损坏配置可能导致无法启动，可用此清除。",
...
const en: typeof zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      finallyLabel: "Cleanup steps (finally)",
      finallyHint: "Runs after the chain; grouped apart in the log.",
      addFinallyAction: "Add cleanup step",
      ...
      actionUpdateProfile: "Update profile bio",
      labelProfileAbout: "New bio",
      profileAboutPlaceholder: "Blank clears the bio",
      profileAboutHint: "Replaces the bio outright; blank clears it. Max 70 chars (140 Premium).",
      actionOpenMiniApp: "Open Mini App (pass Cloudflare)",
      actionOpenMiniAppUrl: "Open Mini App by URL (pass Cloudflare)",
      actionOpenBotMenuApp: "Open the bot's menu Mini App (bottom-left button)",
      actionOpenUrl: "Open web page (pass Cloudflare)",
      ...
      labelCfChallenge: "Solve Cloudflare challenge (I am not a bot)",
      cfChallengeHint: "When the click yields a URL to open (e.g. an \"I am not a bot\" button or link), open it in a headless browser and pass the Cloudflare check to complete the checkin. Requires the image to bundle Chromium.",
      ...
      inAppButtonPlaceholder: "e.g. delay(10000) > Join giveaway|参与抽奖 > Confirm",
      inAppButtonHint: "Run in order once the Mini App page is up, separated by > or ->. Steps available: plain text presses the control carrying that label, preferring a real button or link over a caption that happens to read the same, and several wordings can be given separated by | -- Join giveaway|参与抽奖|加入抽奖 presses whichever the app has, which is how one step covers an app that changes language; css: is a CSS selector, e.g. css:#checkin-btn; delay(10000) waits 10 seconds before the next step (delay(10s) also works), for an app that is slow to settle; scroll(0, 800) scrolls the page by pixels — the first figure is horizontal (x), the second vertical (y), a lone figure is vertical (scroll(800)), and scroll(y=800) / scroll(x=-200) name one axis, with negatives scrolling back — which brings a control that sits off screen into reach; a figure past the end of the page simply lands at the end, so scroll(0, 99999) goes to the bottom; {aiBtn} screenshots the page and lets the AI pick the button to press, with {aiBtn:the checkin button} to say which one; {input} solves an arithmetic captcha and types it; {aiInput} lets the AI read the question and answer it. Common combinations: delay(10000) > {aiBtn} lets the page finish loading before the AI looks at it; scroll(0, 99999) > Check in reaches a button below the fold. Blank auto-detects a checkin-worded control. A disabled or already-completed control is left alone, and if only plain text could be pressed and the app did not react, the action fails rather than reporting a checkin that never happened",
    ...
    web: {
      ...
      timeoutHint:
        "The step fails if the element has not appeared by then. Blank or 0 waits 30 seconds",
  ...
  settings: {
    ...
    feishuSection: "Feishu Notifications",
    feishuHint: "Job-end notifications are also pushed to a Feishu custom bot, sharing the Notify-on setting above with Telegram.",
    labelFeishuWebhook: "Webhook URL",
    feishuWebhookPlaceholder: "https://open.feishu.cn/open-apis/bot/v2/hook/...",
    feishuWebhookHint: "Add a custom bot to a Feishu group and paste its webhook URL. Leave blank to keep the stored one.",
    feishuConfigured: "Feishu push configured",
    labelFeishuSecret: "Signing secret (optional)",
    feishuSecretPlaceholder: "Set it if the bot enforces signature verification",
    feishuSecretPlaceholderSet: "Saved (leave blank to keep)",
    feishuSecretHint: "Required only if the bot's security settings turn on signature verification; otherwise leave blank.",
    feishuTestBtn: "Send test",
    feishuTesting: "Sending…",
    feishuTestSent: "Feishu test notification sent",
    feishuTestFailed: "Feishu test notification failed",
    cfSolver: {
      ...
      clearProfilesHint: "Deletes the cookies and cache each exit has built up. The fingerprint is unaffected, so every exit comes back as the same machine; clearing helps when a profile left behind by a killed browser stops Chromium starting.",
